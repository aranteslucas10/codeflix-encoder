...
func (r *RabbitMQ) Connect() *amqp.Channel {
	...
	connetion_string := "amqp://" + r.User + ":" + r.Password + "@" + r.Host + ":" + r.Port + r.Vhost

	connection, err := amqp.Dial(connetion_string)
	failOnError(err, "Failed to connect to RabbitMQ")

	r.Channel, err = connection.Channel()
	failOnError(err, "Failed to connect to RabbitMQ")
...
func (r *RabbitMQ) Consume(messageChannel chan amqp.Delivery) {
	...
	failOnError(err, "failed to delcare a queue")
