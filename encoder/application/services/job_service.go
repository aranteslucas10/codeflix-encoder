...
func (j *JobService) Start() error {
	...
	err = j.VideoService.Download(os.Getenv("NOT_PROCESSED_VIDEO_BUCKET"))
	log.Printf("j.VideoService.Download error: %v", err)
	if err != nil {
		return j.failJob(err)
	}
...
func (j *JobService) performUpload() error {
	...
	videoUpload.VideoPath = path.Join(os.Getenv("LOCAL_STORAGE_PATH"), j.VideoService.Video.ID)
	concurrency, _ := strconv.Atoi(os.Getenv("CONCURRENCY_UPLOAD"))
	doneUpload := make(chan string)

	log.Printf("Concurrency: %v", concurrency)

	log.Printf("Iniciando processo de upload.")
