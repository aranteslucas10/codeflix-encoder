...
func (j *JobManager) Start(ch *amqp.Channel) {
	...
	for jobResult := range j.JobReturnChannel {
		log.Printf("jobResult Error: %v", jobResult.Error)

		if jobResult.Error != nil {
			err = j.checkParseError(jobResult)
		} else {
...
func (j *JobManager) checkParseError(jobResult JobWorkerResult) error {
	if jobResult.Job.ID != "" {
		log.Printf("MesageID: %v. Error during the job: %v with video: %v. Error: %v", jobResult.Message.DeliveryTag, jobResult.Job.ID,
			jobResult.Job.Video.ID, jobResult.Error.Error())
	} else {
		log.Printf("MesageID: %v. Error parsing message: %v", jobResult.Message.DeliveryTag, jobResult.Error)
	}
